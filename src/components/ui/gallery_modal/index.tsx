    ...
    switch (type) {
      case 'prev':
        ...
        setCurrentPhoto(prev => {
          if (prev - 1 < 0) return album.photos.length - 1
          return prev - 1
        })
        break
      case 'next':
        setDirection(1)
        setCurrentPhoto(prev => {
          if (prev + 1 > album.photos.length - 1) return 0
          return prev + 1
        })
        break
    }
    ...
    <S.Container>
      <S.Wrapper>
        ...
        <AnimatePresence initial={false} custom={direction}>
          ...
          <S.ModalControl>
            <div className="prev" onClick={() => handleChangePhoto('prev')}>{"‣"}</div>
            <div className="next" onClick={() => handleChangePhoto('next')}>{"‣"}</div>
          </S.ModalControl>
        </AnimatePresence>
      </S.Wrapper>
    </S.Container>
